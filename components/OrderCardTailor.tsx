...
import { orderState, tailorOrderState } from "@/utils/orderState";
import { ContactButton } from "./order-button/ContactButton";
import { AssignWorkButton } from "./order-button/AssignWorkButton";
import { ShippingTailorButton } from "./order-button/ShippingTailorButton";

export default function OrderCardTailor({ order }: { order: IOrder }) {
    ...
    const status = {
        ...
        success_tailor: <ShippingTailorButton order_id={order.order_id} />,
    ...
    return (
        <>
            {order.status !== orderState.processing_user && <TouchableOpacity disabled={order?.status === orderState.cancel} onPress={() => router.push(`/tailor-stack/order-detail/${order.order_id}`)} style={{ flexDirection: 'row', height: 90, width: '100%', borderBottomWidth: 0.5, borderColor: colors.grey, paddingBottom: 15, paddingHorizontal: 15, marginTop: 10, opacity: order?.status === orderState.cancel ? 0.5 : 1 }}>
