...
export default function TrackingNumber() {
    ...
    const createTwoButtonAlert = () => {
        Alert.alert('ยืนยันการจัดส่งสินค้า', 'ก่อนกดยืนยันการจัดส่งสินค้า กรุณาตรวจสอบเลขพัสดุและช่องทางการจัดส่งถูกต้อง', [
            {
                text: 'ยกเลิก',
            ...
            },
            {
                text: 'ยืนยันการจัดส่งสินค้า', onPress: async () => {
                    ...
                    await axios.post(process.env.EXPO_PUBLIC_API_URL + '/api/order/update/tracking', { 
                        ...
                        status: order?.status === orderState.success_shop ? orderState.received_user : orderState.received_tailor 
                    }).then(async (res) => {
                        if (res.status === 204) {
                            console.log('success');
